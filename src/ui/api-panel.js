...
export function createApiPanel({ host, onBack, onClose, onCycleTheme, themeIcon, prompt, confirm }) {
  ...
  function render() {
    ...
    for (const profile of data.profiles) {
      ...
      card.append(node('h3', '', profile.name), node('p', 'pcm-snapshot-summary', profile.connection.custom_url), node('p', '', profile.model), node('small', '', profile.secretId ? (secret ? `密钥：${secret.masked || '••••••••'}` : '密钥引用失效，请编辑重新选择') : '无密钥'));
